...
```bash
PG1_IP=172.31.95.101 # set primary IP of pg1
REPL_USER=replicator
# start replication as pg3
sudo -u postgres pg_basebackup -h $PG1_IP -D $MNT/pg17 -U $REPL_USER -P -R --slot=pg3
...
# allow replication user from primary subnet
host    replication     replicator      172.31.0.0/16           scram-sha-256

# allow local TCP for maintenance
...
sudo systemctl restart postgresql@17-main || sudo -u postgres /usr/lib/postgresql/17/bin/postgres -D /pgdata/pg17 -c config_file=/pgdata/pg17/postgresql.conf
...
primary_conninfo = 'user=replicator password=strongpass host=172.31.95.101 port=5432 application_name=pg3'
...
sender_port | 5432
conninfo    | user=replicator password=******** channel_binding=prefer dbname=replication host=172.31.95.101 port=5432 application_name=pg3 fallback_application_name=walreceiver sslmode=prefer sslnegotiation=postgres sslcompression=0 sslcertmode=allow sslsni=1 ssl_min_protocol_version=TLSv1.2 gssencmode=prefer krbsrvname=postgres gssdelegation=0 target_session_attrs=any load_balance_hosts=disable
...
-[ RECORD 1 ]-----------+----------
pg_last_wal_receive_lsn | 1/8F51F08
pg_last_wal_replay_lsn  | 1/8F51F08
